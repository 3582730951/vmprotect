...
[[nodiscard]] bool parse_u8(std::string_view text, std::uint8_t& value_out) noexcept {
  ...
  char* end = nullptr;
  errno = 0;
  const unsigned long parsed = std::strtoul(owned_text.c_str(), &end, 0);
...
[[nodiscard]] bool is_valid_dex_magic(const std::vector<std::uint8_t>& dex_data,
                                      std::array<char, 4u>& version_out) noexcept {
  if (dex_data.size() < 8u) {
    return false;
  }
...
[[nodiscard]] std::string build_manifest_json(const std::array<char, 4u>& dex_version,
  ...
  json += "  \"target_kind\":\"android_dex_research\",\n";
  json += "  \"backend_kind\":\"dex_loader_vm\",\n";
  json += "  \"format\":\"eippf.dex.bundle.v2\",\n";
  json += "  \"dex_version\":\"";
  json += dex_version.data();
  json += "\",\n";
...
int main(int argc, char** argv) {
  ...
  if (!read_binary_file(options.input_path, dex_data)) {
    std::cerr << "[dex_toolchain] failed to read input: " << options.input_path << '\n';
    return 3;
  }

  std::array<char, 4u> dex_version{};
  if (!is_valid_dex_magic(dex_data, dex_version)) {
  ...
  if (key_error != KeyProviderError::kOk) {
    switch (key_error) {
      case KeyProviderError::kReadFailed:
        std::cerr << "[dex_toolchain] key provider read failed: " << options.key_provider_path << '\n';
        return 7;
      case KeyProviderError::kMalformed:
        std::cerr << "[dex_toolchain] key provider is malformed\n";
        return 8;
      case KeyProviderError::kProviderRejected:
        std::cerr << "[dex_toolchain] key provider reported failure\n";
        return 9;
      case KeyProviderError::kKeyIdMismatch:
        std::cerr << "[dex_toolchain] key id mismatch for external provider\n";
        return 10;
  ...
  std::vector<std::uint8_t> encrypted = dex_data;
  encrypt_in_place(encrypted, external_key);
  secure_zero(dex_data);

  std::vector<std::uint8_t> bundle;
  bundle.reserve(24u + options.key_id.size() + encrypted.size());
  bundle.push_back(static_cast<std::uint8_t>('E'));
  bundle.push_back(static_cast<std::uint8_t>('D'));
  bundle.push_back(static_cast<std::uint8_t>('X'));
  ...
  bundle.push_back(static_cast<std::uint8_t>(dex_version[0]));
  bundle.push_back(static_cast<std::uint8_t>(dex_version[1]));
  bundle.push_back(static_cast<std::uint8_t>(dex_version[2]));
  bundle.push_back(0u);  // flags
  ...
  if (!write_binary_file(options.output_bundle_path, bundle)) {
    std::cerr << "[dex_toolchain] failed to write bundle: " << options.output_bundle_path << '\n';
    return 5;
  }

  const std::string manifest =
      build_manifest_json(dex_version, dex_data.size(), bundle.size(), true, options.key_id);
  if (!write_text_file(options.manifest_path, manifest)) {
    std::cerr << "[dex_toolchain] failed to write manifest: " << options.manifest_path << '\n';
    return 6;
  }
