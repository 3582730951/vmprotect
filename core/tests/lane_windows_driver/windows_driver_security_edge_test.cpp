...
[[nodiscard]] std::filesystem::path make_temp_dir() {
  ...
  const std::filesystem::path path = std::filesystem::temp_directory_path() /
                                     ("eippf_windows_driver_security_edge_" +
...
int main() {
  ...
  const std::filesystem::path manifest_path = temp_dir / "driver_bad.manifest.json";
  ...
  const std::string bad_manifest =
      ...
      "\"allow_jit\":true,\"allow_runtime_executable_pages\":false,"
  ...
  if (!write_bytes(artifact_path, build_signed_pe_fixture()) || !write_text(manifest_path, bad_manifest)) {
    std::cerr << "[FAIL] cannot write fixtures\n";
    return 1;
  }

  if (!expect(run_audit(artifact_path, report_path, manifest_path, verifier) != 0,
              "windows driver security-edge should fail")) {
    return 1;
  }
  const std::string report = read_text(report_path);
  if (!expect(report.find("kernel_gate_failed") != std::string::npos,
              "report missing kernel_gate_failed")) {
    return 1;
  }
