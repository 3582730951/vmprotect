...
int main() {
  ...
  if (!expect(run_audit(artifact_path, report_path, manifest_path, verifier) != 0,
              "linux kernel failure lane should fail")) {
    return 1;
  }
  const std::string report = read_text(report_path);
  if (!expect(report.find("vermagic_mismatch") != std::string::npos,
              "report missing vermagic_mismatch")) {
    return 1;
  }
