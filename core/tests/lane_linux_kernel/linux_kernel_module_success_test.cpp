...
int main() {
  ...
  const std::filesystem::path manifest_path = temp_dir / "module.manifest.json";
  ...
  const std::string manifest =
      ...
      "\"kernel_compat_profile\":\"vermagic_profile\",\"hvci_profile\":false,"
      "\"vermagic_profile\":true,\"gki_kmi_profile\":false}\n";

  if (!write_bytes(artifact_path, build_signed_elf_fixture()) || !write_text(manifest_path, manifest)) {
    std::cerr << "[FAIL] cannot write fixtures\n";
    return 1;
  }

  if (!expect(run_audit(artifact_path, report_path, manifest_path, verifier) == 0,
              "linux kernel success lane should pass")) {
    return 1;
  }
  const std::string report = read_text(report_path);
  const std::vector<std::string> required_fragments{
      "\"target_kind\": \"linux_kernel_module\"",
      ...
      "\"kernel_compat_profile\": \"vermagic_profile\"",
      "\"vermagic_profile\": true",
